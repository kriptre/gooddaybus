...
**Live site: [gooddaybus.com](https://gooddaybus.com)** - in production, serving real passengers and real bookings.
...
**Bilingual interface built at build time, not at runtime.** The site is served in Ukrainian at `/` and in English at `/en/`. The English pages are generated by `build-i18n.js` from the same Ukrainian templates plus `i18n/en.json`, so there is one source of truth and the production Ukrainian pages are never hand-edited for translation. Interface strings live in dictionaries rather than inline, and a missing key fails the build instead of silently shipping Ukrainian to an English page. The English pages carry `noindex`: there is no English-language demand for this route network, and a thin second locale would only dilute the Ukrainian search results it competes with.

**Translating vendor data, not just the interface.** Cities, station addresses, carrier names, baggage rules and discount names all arrive from the carrier API in Ukrainian, and most of them cannot be translated by dictionary alone. Three strategies are layered: an exonym dictionary keyed by city id for the cases where transliteration gives the wrong word (`Варшава` must be `Warsaw`, not `Varshava`); transliteration to the Ukrainian state standard KMU-2010 - the one used for passports and street signs - for everything else, so an English speaker can read an address aloud and match it to a sign; and pattern parsers for the free text, which extract the numbers out of a baggage rule and rebuild it as an English sentence. Anything a parser cannot recognise is shown in the original Ukrainian with an honest label rather than hidden - a hidden prepayment condition would leave an English passenger believing the booking is free until they reach the bus.
...
| `build-routes.js` | Generates the 12 SEO route pages from `routes.json`, minifies assets, then runs `build-legal.js` and `build-i18n.js` |
| `build-legal.js` | Generates the legal pages from `legal/*.md` |
| `build-i18n.js` | Generates the English pages under `public/en/` and the client dictionary `public/i18n/en.js` |
| `i18n/` | Translation sources: interface strings, city exonyms, address glossary, KMU-2010 transliteration, vendor-text parsers |
...
| `tests/` | Smoke tests against a running server, plus translation and build-output tests that need no server |
...
`tests/smoke.test.js` makes HTTP requests against an already-running server (`npm start` in another terminal first) and writes nothing to the live carrier API. `tests/i18n.test.js` and `tests/build-output.test.js` need no server: they check the transliteration table against the standard, assert the city dictionary holds no entry that transliteration already handles correctly, and verify the generated English pages contain no visible Cyrillic, carry `noindex`, stay out of `sitemap.xml`, and link only to pages that exist.

## Repository notes

Code comments are in Ukrainian; this README is the English entry point.

## License

Copyright (c) 2026 Illia Reshtanenko. All rights reserved. The source is published for reading and review; it is not licensed for reuse. See [LICENSE](LICENSE).
