...
(function (root) {
    ...
    // "6-а поїздка" -> "6th trip". Українською порядковий номер уже в рядку, англійською
    // потрібен свій суфікс.
    function ordinal(n) {
        var v = parseInt(n, 10);
        var mod100 = v % 100, mod10 = v % 10;
        if (mod100 >= 11 && mod100 <= 13) return v + 'th';
        return v + (mod10 === 1 ? 'st' : mod10 === 2 ? 'nd' : mod10 === 3 ? 'rd' : 'th');
    }

    var DISCOUNT_RULES = [
        // Повний / стандартний / дорослий тариф - це не знижка, а базова ціна
        { re: /^(повний|стандартн|дорослий)/i, en: function () { return 'Full fare'; } },
        // Пільга дітям загиблих захисників. Формулювання передаємо прямо, без пом'якшення:
        // це не маркетингова категорія, і евфемізм тут був би неповагою.
        { re: /загиблих\s+захисник/i, en: function () { return 'Children of fallen defenders of Ukraine'; } },
        // Перевезення тварини - у переліку знижок трапляється як окрема позиція
        { re: /тварин/i, en: function () { return 'Pets'; } },
        // "Молодь від 12 до 18 р." - той самий віковий діапазон, інше слово
        { re: /молод[ья][^\d]*(\d+)\s*(?:до|-|–)\s*(\d+)\s*р/i, en: function (m) { return 'Youth ' + m[1] + '-' + m[2]; } },
        // "Понад 60 років" - похилий вік, але сказано через "понад", а не "від"
        { re: /понад\s*(\d+)\s*рок/i, en: function (m) { return 'Seniors (' + m[1] + '+)'; } },
        // "Група від 5 осіб"
        { re: /груп[аи][^\d]*(\d+)\s*осіб/i, en: function (m) { return 'Group of ' + m[1] + ' or more'; } },
        // "6-а поїздка з перевізником" - знижка за повторні поїздки
        { re: /(\d+)\s*-?\s*[ая]?\s*поїздк[аи]\s+з\s+перевізник/i, en: function (m) { return ordinal(m[1]) + ' trip with the carrier'; } },
        // Діапазон віку: "Діти 5-12 років", "Діти 0-5 лет", "Дитячий (діти від 7 до 12 років)"
        { re: /д[иі]т(?:и|ей|ячий)[^\d]*(?:від\s*)?(\d+)\s*(?:-|–|до)\s*(\d+)\s*(?:р|лет)/i, en: function (m) { return 'Children ' + m[1] + '-' + m[2]; } },
        // Верхня межа віку зі шкільним квитком - окремо, бо умова важлива
        { re: /д[иі]т(?:и|ей)[^\d]*(\d+)[^)]*учнівськ/i, en: function (m) { return 'Children up to ' + m[1] + ' (with school ID)'; } },
        // Верхня межа віку: "Діти до 10 років (включно)", "Дитячий (діти до 6 років)"
        { re: /д[иі]т(?:и|ей|ячий)[^\d]*до\s*(\d+)\s*р/i, en: function (m) { return 'Children up to ' + m[1]; } },
