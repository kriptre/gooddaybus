...
test('baggageText: "до двох одиниць" + габарити + вага (реальний рядок)', () => {
    const raw = 'У вартість квитка входить до двох одиниць багажу загальною вагою до 40кг. ' +
        'Додатковий багаж може бути перевезений за окрему плату і тільки при наявності вільного місця в багажному відділенні автобуса.';
    // "загальною вагою" - ліміт на ОБИДВА місця разом, тож "in total" обовʼязкове:
    // без нього пасажир читає 40 кг на місце й пакує 80. Раніше цей тест закріплював
    // саме помилковий вивід без уточнення.
    assert.deepEqual(baggageText(raw), {
        text: 'Ticket price includes up to 2 items of baggage, up to 40 kg in total. ' +
            'Extra baggage can be carried for a separate fee, subject to space in the bus luggage compartment.',
        translated: true
    });
});
...
// "загальною/сумарною вагою" - це ліміт НА ВСІ МІСЦЯ РАЗОМ, а "кожна" - на кожне.
// Без уточнення "up to 2 items of baggage, up to 40 kg" читається як 40 кг на місце:
// пасажир пакує 80 кг і його розвертають при посадці. Та сама порода помилки, що й
// мінімум замість максимуму.
test('багаж: сумарна вага не видається за вагу одного місця', () => {
    const tail = ' Додатковий багаж може бути перевезений за окрему плату і тільки при наявності вільного місця в багажному відділенні автобуса.';

    const total = baggageText('У вартість квитка входить до двох одиниць багажу загальною вагою до 40кг.' + tail);
    assert.equal(total.translated, true);
    assert.match(total.text, /up to 40 kg in total/, 'ліміт на всі місця разом має бути позначений');

    const each = baggageText('У вартість квитка входить дві одиниці багажу вагою до 20кг кожна.' + tail);
    assert.equal(each.translated, true);
    assert.match(each.text, /up to 20 kg each/, 'ліміт на кожне місце має лишитись "each"');
});

test('багаж: для одного місця уточнення "разом" не додається', () => {
    const tail = ' Додатковий багаж може бути перевезений за окрему плату і тільки при наявності вільного місця в багажному відділенні автобуса.';
    const r = baggageText('У вартість квитка входить одна одиниця багажу загальною вагою до 25кг.' + tail);
    assert.equal(r.translated, true);
    // З одним місцем "кожне" і "разом" - те саме, тож уточнення лише заважало б
    assert.doesNotMatch(r.text, /in total|each/);
    assert.match(r.text, /up to 25 kg/);
});

test('багаж: суперечливий рядок ("кожна" і "разом" водночас) відхиляється', () => {
    const tail = ' Додатковий багаж може бути перевезений за окрему плату і тільки при наявності вільного місця в багажному відділенні автобуса.';
    const s = 'У вартість квитка входить дві одиниці багажу загальною вагою до 40кг кожна.' + tail;
    const r = baggageText(s);
