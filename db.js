...
function createOrder(data) {
    ...
    const stmt = db.prepare(`
        INSERT INTO orders
            (created_at, updated_at, status, client_name, client_phone, comment,
             ...
             route_from_station, route_to_station, check_warning, booked, tickets, pet, page, landing, token)
        VALUES
            (?, ?, 'new', ?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    const info = stmt.run(
        ...
        data.pet ? 1 : 0, data.page || '', data.landing || '', data.token || null
    );
...
function getStats(from, to) {
    ...
    return {
        ...
        // "Незадоволений попит" = напрямки, які за період ЖОДНОГО разу не повернули рейсів
        // (HAVING MAX(results)=0). Маршрути, що мають рейси, але порожні на окрему дату
        // (гортання стрічки дат, дата за горизонтом розкладу), сюди НЕ потрапляють - інакше
        // популярні напрямки засмічували б список фальшивим "немає рейсів".
